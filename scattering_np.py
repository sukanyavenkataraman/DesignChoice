...
def stack_real_imag(x):
    ...
    stack_axis = len(x.shape)
    print x.shape
    return np.stack((np.real(x), np.imag(x)), axis=stack_axis)
...
def compute_fft(x, direction="C2C", inverse=False):
    ...
    if direction == 'C2R':
        out = np.real(np.ifft2d(x_complex)) * h * w
        return out

    else:
        if inverse:
            out = stack_real_imag(np.fft.ifft2(x_complex)) * h * w
        else:
            out = stack_real_imag(np.fft.fft2(x_complex))
        return out
...
def cdgmm(A, B):
    ...
    print C_r.shape, C_i.shape
    print np.stack((C_r, C_i), axis=-1).shape
    return np.asarray:q
((C_r, C_i), axis=-1)
...
def modulus(x):
    ...
    out = np.norm(x, axis=len(input_shape) - 1)
...
class Scattering(object):
    ...
    def _pad(self, x):
        ...
        out_ = np.pad(x, paddings, mode="reflect")
        out_ = np.expand_dims(out_, axis=-1)
        output = np.concatenate([out_, np.zeros_like(out_)], axis=-1)
        return output
