...
def filters_bank(M, N, J, L=8):
    ...
    for j in range(J):
        for theta in range(L):
            ...
            for res in range(j + 1):
                ...
                psi[res]=np.stack((np.real(psi_signal_fourier_res), np.imag(psi_signal_fourier_res)), axis=2)
    ...
    for res in range(J):
        ...
        filters['phi'][res]=np.stack((np.real(phi_signal_fourier_res), np.imag(phi_signal_fourier_res)), axis=2)
